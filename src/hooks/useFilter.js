export const useFilter = () => {
    ...
    const filterProducts = (
        ...
        if (searchValue) {
            filteredProducts = filterBySearchValue(
                filteredProducts,
                searchValue
            );
        }

        filteredProducts = callBackOne(filteredProducts, filter);

        if (sort) {
            sortProduct(filteredProducts, sort);
        }

        const {productsCount, paginatedProducts} = callBackTwo(
            filteredProducts,
            pagination
        );
    ...
    return {filterProductsByFilterInfo, filterProducts};
};
